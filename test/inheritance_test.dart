...
void main() {
  test('Atom should implement ValueListenable but NOT extend ValueNotifier', () {
    ...
    expect(atom is ValueNotifier, isFalse, reason: 'Atom should not extend ValueNotifier (Leaky Abstraction fix)');
  });

  test('ComputedAtom should implement ValueListenable but NOT extend ValueNotifier', () {
    final a = Atom(1);
    final c = ComputedAtom(() => a.value * 2);
    
    expect(c, isA<ValueListenable<int>>());
    expect(c is ValueNotifier, isFalse, reason: 'ComputedAtom should not extend ValueNotifier');
  });
